...
float radius = 0.0325;                              //Wheel radius, in m
float wheelbase = 0.2;   
float speed_act_left = 0.0;
float speed_act_right = 0.0;
float two_pi = 6.28319;
float speed_req1 = 0.0;
float speed_req2 = 0.0;
float speed_dt = 0.0; 
float x_pos = 0.0;
float y_pos = 0.0;
float theta = 0.0;
...
void handle_speed(const geometry_msgs::Vector3Stamped& speed){
    ...
    ROS_INFO("speed rightll : %f", speed_act_right);
}
...
int main(int argc, char** argv){
    ...
    ros::Subscriber sub = nh.subscribe("/speed",50, handle_speed);
    ros::Publisher odom_pub = nh.advertise<nav_msgs::Odometry>("odom", 50); 
    tf::TransformBroadcaster broadcaster;  
    double rate = 20.0;//odom publish rate
